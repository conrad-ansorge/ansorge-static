...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="works_bibl" select="document($listbiblPath, .)" as="node()?"/>
    <xsl:variable name="works_work" select="document($listworkPath, .)" as="node()?"/>
    <xsl:variable name="works_fallback">
        <listBibl xmlns="http://www.tei-c.org/ns/1.0"/>
    </xsl:variable>
    <xsl:variable name="works" select="
            if ($works_bibl)
            then $works_bibl
            else if ($works_work)
            then $works_work
